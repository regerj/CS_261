 ...
 */
struct bst_iterator {
  struct stack * in_order;
  struct bst_node * curr_bst_node;
  struct bst * bst;
};
...
int bst_size(struct bst* bst) {
  int count;
  if(bst_isempty(bst) == 1){
    return 0;
  }else{
    struct bst * temp_left = bst_create();
    temp_left->root = bst->root->left;
    struct bst * temp_right = bst_create();
    temp_right->root = bst->root->right;
    count = (bst_size(temp_left) + 1 + bst_size(temp_right));
    free(temp_left);
    free(temp_right);
    return(count);
  }
...
int bst_height(struct bst* bst) {
  if(bst_isempty(bst) == 1){
    ...
    int left_height = 0;
    int right_height = 0;
    left_height = bst_height(temp_left);
    right_height = bst_height(temp_right);
...
int bst_path_sum(int sum, struct bst* bst) {
  ...
  if(bst_isempty(bst) == 1){
    ...
    int int_exists = 0;
    int temp = 0;
    temp = sum - bst->root->val;

    if(temp == 0 && is_leaf(bst) == 1){
      free(temp_left);
      free(temp_right);
      return 1;
    }
    if(bst->root->left != NULL){
      if(int_exists == 1 || bst_path_sum(temp, temp_left) == 1){
        int_exists = 1;
      }else{
        int_exists = 0;
      }
    ...
    if(bst->root->right != NULL){
      if(int_exists == 1 || bst_path_sum(temp, temp_right) == 1){
        int_exists = 1;
      }else{
        int_exists = 0;
      }
...
void bst_iterator_fill(struct bst_iterator * iter){
  if(stack_isempty(iter->in_order) == 1){
    struct bst_node * temp = iter->bst->root;
    while(temp != NULL){
      stack_push(iter->in_order, temp);
      temp = temp->left;
    }
  }else{
    struct bst_node * temp2 = stack_pop(iter->in_order);
    struct bst_node * temp3 = temp2->right;
    while(temp3 != NULL){
      stack_push(iter->in_order, temp3);
      temp3 = temp3->left;
    }
  }
}
...
struct bst_iterator* bst_iterator_create(struct bst* bst) {
  struct bst_iterator * bst_iter = malloc(sizeof(struct bst_iterator));
  bst_iter->in_order = stack_create();
  bst_iter->bst = bst;
  bst_iter->curr_bst_node = bst->root;
...
int bst_iterator_next(struct bst_iterator* iter) {
  int * temp = (int *)(stack_top(iter->in_order));
  return *temp;
}
