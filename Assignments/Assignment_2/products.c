...
struct dynarray* create_product_array(int num_products, char** names, int* inventory, float* prices) {
  ...
  _dynarray_resize(temp, num_products);
  for(int i = 0; i < num_products; i++){
    struct product * prodtemp = create_product(names[i], inventory[i], prices[i]);
    dynarray_insert(temp, i, prodtemp);
  }
  return temp;
}
...
void print_products(struct dynarray* products) {
  for(int i = 0; i < dynarray_length(products); i++){
    struct product * temp = (struct product *) dynarray_get(products, i);
    int templength = strlen(temp->name);
    if(templength > 9){
      printf("Product #%i\t\tName: %s\tInventory: %i\t\tPrice: %.2f\n", i+1, temp->name, temp->inventory, temp->price);
    }else{
      printf("Product #%i\t\tName: %s\t\tInventory: %i\t\tPrice: %.2f\n", i+1, temp->name, temp->inventory, temp->price);
    }
  }
...
struct product* find_max_price(struct dynarray* products) {
  struct product * maxprice = NULL;
  struct product * temp = (struct product *) dynarray_get(products, 0);
  maxprice = temp;
  for(int i = 1; i < dynarray_length(products); i++){
    struct product * temp2 = (struct product *) dynarray_get(products, i);
    if(maxprice->price < temp2->price){
      maxprice = temp2;
    }
  }
  return maxprice;
...
struct product* find_max_investment(struct dynarray* products) {
  struct product * maxinv = NULL;
  struct product * temp = (struct product *) dynarray_get(products, 0);
  maxinv = temp;
  for(int i = 1; i < dynarray_length(products); i++){
    struct product * temp2 = (struct product *) dynarray_get(products, i);
    if(maxinv->price * maxinv->inventory < temp2->price * temp2->inventory){
      maxinv = temp2;
    }
  }
  return maxinv;
...
void sort_by_inventory(struct dynarray* products) {
  for(int i = 0; i < dynarray_length(products) - 1; i++){
    for(int j = 0; j < dynarray_length(products) - i - 1; j++){
      struct product * temp = (struct product *) dynarray_get(products, j);
      struct product * temp2 = (struct product *) dynarray_get(products, j + 1);
      if(temp->inventory > temp2->inventory){
        //struct product * temp3 = temp;
        dynarray_set(products, j, temp2);
